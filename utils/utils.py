...
def log_validation(
    ...
    for idx, (validation_prompt, validation_image) in enumerate(zip(validation_prompts, validation_images)):
        ...
        for i in range(args.num_validation_images):
            with inference_ctx:
                image = pipeline(
                    validation_prompt, validation_image, num_inference_steps=args.num_inference_steps, generator=generator, guidance_scale=args.guidance_scale,
                ).images[0]
            
            images.append(image)
            
            # Save the generated image
            os.makedirs(os.path.join(args.output_dir, "validation_images", validation_prompt.replace(" ", "-")), exist_ok=True)
            image.save(os.path.join(args.output_dir, "validation_images", validation_prompt.replace(" ", "-"), f"step_{step}.png"))
